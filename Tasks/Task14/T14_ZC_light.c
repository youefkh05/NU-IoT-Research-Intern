 ...
 * GLOBAL VARIABLES
 */
 uint8_t sensor_receivedValue = 0;
    ...
    switch(pMsg->hdr.event)
    {
        ...
        case zstackmsg_CmdIDs_ZDO_DEVICE_ANNOUNCE:
        {
           zstackmsg_zdoDeviceAnnounceInd_t *pInd =
            ...
            dstAddr.addr.shortAddr = pInd->req.srcAddr;
            dstAddr.endPoint = 1;       // if your ZED endpoint is 1
            dstAddr.panId = 0;
/*
        ...
        case zstackmsg_CmdIDs_ZDO_IEEE_ADDR_RSP:
        {
            ...
            if(pInd->rsp.status == zstack_ZdpStatus_SUCCESS)
            {
                ...
                // Send a test Level Control command
                zclGeneral_SendLevelControlMoveToLevel(
                    SAMPLELIGHT_ENDPOINT,
                    &Req.bindInfo.dstAddr,
                    100,            // level
                    0,              // transition time
                    FALSE,
                    seqNum++);
  ...
  switch(bdbCommissioningModeMsg->bdbCommissioningMode)
  {
    ...
    case BDB_COMMISSIONING_NWK_STEERING:
      if(bdbCommissioningModeMsg->bdbCommissioningStatus == BDB_COMMISSIONING_SUCCESS)
      {
        ...
        zclGeneral_SendLevelControlMoveToLevel(
            SAMPLELIGHT_ENDPOINT,      // Source endpoint
            &zclSampleLight_DstAddr,   // Destination
            100,                       // Level (0-254)
            0,                         // Transition time
            FALSE,                     // Default response enabled
...
/*
  CUI_statusLinePrintf(gCuiHandle,
                     gSampleLightInfoLine,
                     "Received cmd = %d", cmd);
*/
  // Turn on the light
...
static void zclSampleLight_LevelControlMoveToLevelCB( zclLCMoveToLevel_t *pCmd )
{
  ...
  sensor_receivedValue = pCmd->level;

  Display_printf(dispHandle, 0, 0,
               "Received level = %d", pCmd->level);
...
static void zclSampleLight_UpdateStatusLine(void)
{
    ...
    strcpy(lineFormat, "["CUI_COLOR_YELLOW"Light rece"CUI_COLOR_RESET"]");
    // set the LED1 based on light (on or off)
    if ( LIGHT_ON == zclSampleLight_getOnOffAttribute())
    {
...
#ifdef ZCL_LEVEL_CTRL
    strcat(lineFormat,
       " ["CUI_COLOR_YELLOW"Level"CUI_COLOR_RESET"] %03d"
       " ["CUI_COLOR_YELLOW"Received"CUI_COLOR_RESET"] %03d");
       
    CUI_statusLinePrintf(gCuiHandle,
                     gSampleLightInfoLine,
                     lineFormat,
                     zclSampleLight_getCurrentLevelAttribute(),
                     sensor_receivedValue);

    Display_printf(dispHandle, 0, 0,
               "Received level = %d", 5); // didn't work
